...
fn apply_common(cmd: &mut CommandBuilder, cwd: Option<String>) {
    ...
    cmd.env("KAI_TERMINAL", "1");
    ensure_utf8_locale(cmd);

    let resolved_cwd = cwd
        .map(PathBuf::from)
        .filter(|p| p.is_dir())
        // Prefer home dir over process cwd. In installed builds the process
        // cwd is the install directory (e.g. C:\Program Files\KAI) which
        // is not a useful starting point and can cause permission issues on
        // some shells. In `tauri dev` the launch-dir mechanism seeds the
        // first tab's cwd from the repo root anyway.
...
mod unix {
    ...
    fn integration_root() -> Result<PathBuf, String> {
        ...
        let root = home.join(".cache").join("KAI").join("shell-integration");
    ...
    fn prepare_fish_conf_d() -> Result<(), String> {
        ...
        fs::create_dir_all(&dir).map_err(|e| format!("create {}: {e}", dir.display()))?;
        write_if_changed(&dir.join("KAI.fish"), FISH_INIT)?;
        Ok(())
    }
...
mod windows {
    ...
    pub fn build(cwd: Option<String>, workspace: WorkspaceEnv, shell: Option<String>) -> Result<CommandBuilder, String> {
        if let WorkspaceEnv::Wsl { distro } = workspace {
            return build_wsl(cwd, distro);
        }
        let shell_path = super::resolve_windows_shell(shell.as_deref());
        ...
        match (shell_kind, integration) {
            (
                ...
                if let Some(user_zdotdir) = user_zdotdir {
                    args.push(format!("KAI_USER_ZDOTDIR={user_zdotdir}"));
                }
    ...
    fn prepare_wsl_integration_dir(distro: &str, shell: &str) -> Result<(String, PathBuf), String> {
        ...
        let linux_dir = format!(
            "{}/.cache/KAI/shell-integration/{shell}",
    ...
    fn prepare_wsl_fish_conf_d(distro: &str) -> Result<(), String> {
        ...
        let unc_dir = crate::modules::workspace::wsl_path_to_unc(distro, &linux_dir);
        fs::create_dir_all(&unc_dir).map_err(|e| format!("create {}: {e}", unc_dir.display()))?;
        let unc_file = unc_dir.join("KAI.fish");
    ...
    mod tests {
        ...
        fn builds_wsl_zsh_launch_spec_with_env_and_login() {
            let spec = build_wsl_launch_spec(
                ...
                ShellKind::Zsh,
                WslShellIntegration::Zsh {
                    zdotdir: "/home/vinicios/.cache/KAI/shell-integration/zsh".into(),
                    user_zdotdir: None,
                },
            );
            assert_eq!(
                ...
                vec![
                    ...
                    "--exec".to_string(),
                    "env".to_string(),
                    "ZDOTDIR=/home/vinicios/.cache/KAI/shell-integration/zsh".to_string(),
        ...
        fn builds_wsl_zsh_launch_spec_with_user_zdotdir_probe() {
            ...
            assert_eq!(
                ...
                vec![
                    ...
                    "--exec".to_string(),
                    "env".to_string(),
                    "KAI_USER_ZDOTDIR=/home/vinicios/.config/zsh".to_string(),
                    "ZDOTDIR=/home/vinicios/.cache/KAI/shell-integration/zsh".to_string(),
                    "/usr/bin/zsh".to_string(),
                    "-l".to_string(),
                ]
        ...
        fn builds_wsl_bash_launch_spec_with_rcfile() {
            let spec = build_wsl_launch_spec(
                ...
                WslShellIntegration::Bash {
                    rcfile: "/home/vinicios/.cache/KAI/shell-integration/bash/bashrc".into(),
                },
            );
            assert_eq!(
                spec.args,
                vec![
                    ...
                    "/bin/bash".to_string(),
                    "--rcfile".to_string(),
                    "/home/vinicios/.cache/KAI/shell-integration/bash/bashrc".to_string(),
                    "-i".to_string(),
                ]
            );
        }
...
/// Resolve a shell path from a user preference string.
/// "auto" or empty = auto-detect. "cmd" / "powershell" / "pwsh" = well-known.
/// Anything else = treat as an absolute path.
#[cfg(windows)]
pub fn resolve_windows_shell(pref: Option<&str>) -> PathBuf {
    match pref.unwrap_or("auto").trim() {
        "" | "auto" => windows_shell_path(),
        "cmd" => {
