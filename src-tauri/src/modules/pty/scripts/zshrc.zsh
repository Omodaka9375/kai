...
# zsh, so we shadow $? into `_KAI_ret`.

{
  _KAI_user_zdotdir="${KAI_USER_ZDOTDIR:-$HOME}"
  [ -f "$_KAI_user_zdotdir/.zshrc" ] && source "$_KAI_user_zdotdir/.zshrc"
...
if [[ -z "$__KAI_HOOKS_LOADED" ]]; then
  __KAI_HOOKS_LOADED=1
  ...
  _KAI_precmd() {
    local _KAI_ret=$?
    printf '\e]133;D;%s\e\\' "$_KAI_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_KAI_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _KAI_precmd
    add-zsh-hook preexec _KAI_preexec
  fi
