...
if [ -z "$__KAI_HOOKS_LOADED" ]; then
  ...
  _KAI_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_KAI_urlencode "$PWD")"
    if [ -z "$__KAI_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __KAI_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_KAI_precmd:"*) ;;
    *) PROMPT_COMMAND="_KAI_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
