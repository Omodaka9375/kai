...
export function useTabs(initial?: Partial<TerminalTab> & { defer?: boolean }) {
  const [tabs, setTabs] = useState<Tab[]>(() => {
    if (initial?.defer) return [];
    const tabId = 1;
    const leafId = 2;
    return [
      {
  ...
  const [activeId, setActiveId] = useState(initial?.defer ? 0 : 1);
  ...
  const newApiTesterTab = useCallback(() => {
    ...
    setTabs((t) => [
      ...t,
      { id, kind: "api-tester", title: "API Tester" } satisfies ApiTesterTab,
    ]);
    setActiveId(id);
    return id;
  }, []);
  ...
  const openMarkdownPreview = useCallback((path: string) => {
    ...
    setTabs((curr) => {
      const existing = curr.find(
        (t) => t.kind === "md-preview" && t.path === path,
      );
      if (existing) {
        targetId = existing.id;
        return curr;
