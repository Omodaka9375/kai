  ...
  return {
    batch_edit: tool({
      ...
      execute: async ({ edits }): Promise<BatchEditResult> => {
        // Phase 1: resolve and validate all paths. Collect absolute paths
        // so we can lock them all before touching any file.
        ...
        for (const edit of edits) {
          ...
          absPaths.push(safety.canonical);
        }

        return withFileMutationLock(absPaths, async () => {
          ...
          for (const abs of absPaths) {
            try {
              ...
              if (r.kind === "text") {
                ...
                return { ok: false, error: `binary file: ${abs}`, file: abs };
              }
            } catch {
          ...
          // Phase 3: apply all edits in memory (re-read snapshots for
          // each edit so same-file edits compound correctly).
          for (let ei = 0; ei < edits.length; ei++) {
            const edit = edits[ei];
            const abs = absPaths[ei];
          ...
          // Phase 4: write all results to disk.
