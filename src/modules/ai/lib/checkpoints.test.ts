...
// records are plain JSON outside the workspace now, but the paths they name are
// still attacker-controllable (e.g. a hand-planted checkpoint record naming
// `~/.ssh/authorized_keys` would otherwise be written verbatim, with no approval
...
describe("confineToWorkspace — escapes are rejected", () => {
  ...
  it("rejects attempts to overwrite the checkpoint store itself", () => {
    // Restoring into the legacy in-workspace `.kai/checkpoints` location is how
    // a payload re-seeds itself.
    ...
    // And into the `.kai` state root generally — agent state, never a restore
    // target.
    expect(confineToWorkspace("/proj", "/proj/.kai/seed.json")).toBeNull();
  });
});
...
describe("confineToWorkspace — legitimate paths are preserved", () => {
  ...
  it("accepts other dot-dirs under the workspace", () => {
    // `.github` etc. are real project files that undo must be able to restore.
