...
export type Preferences = {
  ...
  /** Proxy URL for non-local web preview. `{url}` is replaced with the target. */
  previewProxyUrl: string;
  /** Preferred shell on Windows: "auto", "cmd", "powershell", "pwsh", or a custom path. */
  defaultShell: string;
  /** Last workspace directory — restored on next launch if no launch dir. */
  lastWorkspaceCwd: string;
  /** Context window size in tokens for LM Studio model. 0 = use default (128k). */
...
const KEY_PREVIEW_PROXY_URL = "previewProxyUrl";
...
// window can listen.
const PREFS_CHANGED_EVENT = "Kai://prefs-changed";
...
export async function loadPreferences(): Promise<Preferences> {
  ...
  return {
    ...
    previewProxyUrl:
      get<string>(KEY_PREVIEW_PROXY_URL) ??
      DEFAULT_PREFERENCES.previewProxyUrl,
    defaultShell:
      get<string>(KEY_DEFAULT_SHELL) ??
      DEFAULT_PREFERENCES.defaultShell,
    lastWorkspaceCwd:
      get<string>(KEY_LAST_WORKSPACE_CWD) ??
      DEFAULT_PREFERENCES.lastWorkspaceCwd,
    lmstudioContextSize:
      get<number>(KEY_LMSTUDIO_CTX_SIZE) ??
      DEFAULT_PREFERENCES.lmstudioContextSize,
...
export async function setPreviewProxyUrl(value: string): Promise<void> {
  await writePref(KEY_PREVIEW_PROXY_URL, value);
}
...
export async function setLmstudioContextSize(value: number): Promise<void> {
  await writePref(KEY_LMSTUDIO_CTX_SIZE, value);
}

export async function setOpenaiCompatibleContextSize(value: number): Promise<void> {
...
export async function setLastWorkspaceCwd(value: string): Promise<void> {
  ...
  await store.save();
  // No event emit — this is written from the main window only, no
...
export async function onPreferencesChange(
  ...
  const map: Record<string, PrefKey> = {
    ...
    [KEY_PREVIEW_PROXY_URL]: "previewProxyUrl",
...
const KEYS_CHANGED_EVENT = "Kai://ai-keys-changed";
