...
export function CwdBreadcrumb({ cwd, filePath, home, onCd }: Props) {
  ...
  if (filePath) {
    ...
    return (
      <Breadcrumb>
        <BreadcrumbList className="gap-0.5 text-xs sm:gap-1">
          {first ? (
            <BreadcrumbSegment
              label={first.label}
              isHome={first.isHome}
      ...
      <BreadcrumbItem>
        <BreadcrumbLink asChild>
          <button
            type="button"
            onClick={onClick}
            className="flex cursor-pointer items-center gap-1 rounded px-1 py-0.5 text-muted-foreground transition-colors hover:text-foreground"
          >
            {isHome ? (
              <HugeiconsIcon
                icon={Home03Icon}
