...
- **Agent** (`lib/agent.ts`): drives `streamText` directly (the stable AI SDK primitive — not the `Experimental_Agent` wrapper) with `stopWhen: stepCountIs(MAX_AGENT_STEPS)` and the system prompt from `config.ts`. KAI hand-rolls the agent loop on top of `streamText` so the SDK's experimental agent API churn can't break it, and to keep control over approval cards, tool fencing/redaction, and loop detection (`onStepFinish`). Provider branching happens here (model built via `buildConfiguredLanguageModel`/`buildLanguageModel`); the rest of the system depends on AI SDK v6 chat semantics. Sub-agents use `generateText` the same way (`runSubagent.ts`).
