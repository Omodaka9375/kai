...
KAI loads `KAI.md` from the workspace root as agent memory (similar to AGENTS.md / CLAUDE.md). This file is also the project's living architecture doc — read it before making changes.
...
- **Windows** (`profile.ps1`) — passed via `pwsh -NoLogo -NoExit -ExecutionPolicy Bypass -File <path>`. Since `-File` suppresses automatic `$PROFILE` loading, the script explicitly sources `$PROFILE` first (with error handling) so the user gets their full environment (Oh-My-Posh, Starship, posh-git, custom modules). It then captures the user's `prompt` function and wraps it to emit OSC 7 + OSC 133 A/B/D. Shell priority: `pwsh.exe` (PS 7+) → `powershell.exe` (PS 5.1) → `cmd.exe` (no integration). cwd is normalized to backslashes before being passed to ConPTY (`CreateProcessW` misbehaves with forward-slash cwd).
...
**Process-wide kill-on-close job (Windows)**: `install_process_wide_kill_on_close()` in `pty/job.rs`, called first thing in `Builder::setup`. It assigns the KAI process itself to a Job with `KILL_ON_JOB_CLOSE` and leaks the handle for the process lifetime. This is the conhost backstop: ConPTY's conhost is parented to the KAI process, NOT to the shell, so the per-session Job never covered it — every abnormal KAI death (crash, dev Ctrl-C, taskkill) leaked one conhost per open tab. Children inherit the process-wide job automatically; nested jobs (Win8+) keep the per-session jobs working. macOS/Linux rely on `Drop for Session → killer.kill()`; orphans are still possible there on dev-`Ctrl-C` since destructors don't fire.

**PTY spawn failure paths** (`pty/session.rs`): every error after `spawn_command` succeeds must kill the live child and drop the ConPTY master **on a background thread** (`cleanup_failed_spawn` / the three thread-spawn `.map_err` arms). `MasterPty`'s Drop runs `ClosePseudoConsole`, which can block for hundreds of ms — cleaning up inline on the IPC worker thread stalls every later `pty_open` behind `SPAWN_LOCK` and trips the frontend's 20s watchdog retry loop (the long-standing "failed spawn" shell accumulation).
...
BYOK. Multi-provider via `@ai-sdk/*`: **OpenAI, Anthropic, Google, Groq, xAI, Cerebras, DeepSeek, Mistral, OpenRouter, z.ai, OpenAI-compatible, LM Studio** (last two for local/offline; DeepSeek/Mistral/OpenRouter/z.ai ride the openai-compatible client with fixed base URLs). Provider list in `config/providers.ts` (`PROVIDERS`, 12 entries; barrel `config/index.ts` re-exports providers/models/runtime/prompt); model registry in `config/models.ts` includes `DEFAULT_MODEL_ID` + `DEFAULT_AUTOCOMPLETE_MODEL`. The OpenRouter model list auto-syncs at runtime. The OpenRouter model list auto-syncs at runtime.
...
- **Agent** (`lib/agent.ts`): drives `streamText` directly (the stable AI SDK primitive — not the `Experimental_Agent` wrapper) with `stopWhen: stepCountIs(MAX_AGENT_STEPS)` and the system prompt from `config/prompt.ts`. KAI hand-rolls the agent loop on top of `streamText` so the SDK's experimental agent API churn can't break it, and to keep control over approval cards, tool fencing/redaction, and loop detection (`onStepFinish`). Provider branching happens here (model built via `buildConfiguredLanguageModel`/`buildLanguageModel`); the rest of the system depends on AI SDK v6 chat semantics. Sub-agents use `generateText` the same way (`runSubagent.ts`).
...
- Auto-updater configured with a public minisign key; release artifacts at `https://github.com/interlooper/kai/releases/latest/download/latest.json`.
...
- **Windows PowerShell process lifecycle**: `killer.kill()` from `portable-pty` only kills the immediate child. Descendants (e.g. `npm run dev` started inside pwsh) survive unless something else takes them down. The Job Object in `pty/job.rs` handles this for the KAI-process-death case; an explicit `pty_close` from JS also kills only the immediate child + relies on the Job to take the rest. Don't disable the Job without a replacement.
